...
class _Dial extends StatefulWidget {
  ...
  final Function(Duration, BuildContext)? onComplete;
  final BaseUnit baseUnit;
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  void _handlePanEnd(DragEndDetails details) {
    ...
    widget.onComplete?.call(widget.duration, context);
  }
...
class DurationPickerDialogState extends State<DurationPickerDialog> {
  ...
  void _handleTimeChanged(Duration value) {
    if (widget.maxTime != null && value.inMilliseconds > widget.maxTime!.inMilliseconds) {
  ...
  Widget build(BuildContext context) {
    ...
    final dialog = RawKeyboardListener(
      autofocus: true,
      focusNode: FocusNode(),
      onKey: (RawKeyEvent event) {
        if (widget.showOkButton == true && event.logicalKey == LogicalKeyboardKey.enter) {
          if (Navigator.canPop(context)) _handleOk();
        }
      },
      child: Dialog(
...
Future<Duration?> showDurationPicker({
  ...
  void Function(Duration, BuildContext)? onComplete,
